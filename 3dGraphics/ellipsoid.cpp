...
Ellipsoid::Ellipsoid(float A, float B, float C, float X0, float Y0, float Z0, float Alpha) {
    ...
    for (double x = -pole; floor(x * 10000 + 0.5) / 10000 < pole; x += (double)pole * 2 / (double)prec_x) {
        ...
        for (double y = -pole; floor(y * 10000 + 0.5) / 10000 < pole; y += (double)pole * 2 / (double)prec_y) {
            if (1 - a * pow(x + x0, 2) - b * pow(y + y0, 2) > -1) {
                setX(i, x);
                setY(j, y);
                setZ(i, j, sqrt(1 - a * pow(x + x0, 2) - b * pow(y + y0, 2)) / c - z0);
                j++;
            }
        }
        i++;
